...
Two findings worth recording, because both were the kind of mistake that does not announce
itself.

**The backend was more permissive than the browser.** `als_zahl` was leaving the parse to
Python's `float()`, which accepts numeric underscores, while the browser's `Number()` does not.
A catch cell holding `1_000` was therefore a thousand fish to the authoritative half and an
unreadable cell to the advisory one. The gate being the looser of the two is exactly backwards,
which is the whole reason this sub-feature exists. What counts as a number is now written out as
a pattern rather than delegated, and tested against both directions of the disagreement,
including `0x10`, which `Number()` reads as 16 and `float()` refuses.

**The required-field list had grown past the form's own asterisks.** `probestrecke.untere`,
`probestrecke.obere` and `ausruestung.leistung` carry no `pflicht` marker, and an early grep had
mis-attributed them. The list is meant to be those 32 markers and nothing besides, since widening
it quietly is how the two halves start disagreeing about what a finished protocol is. They are
out, and a test pins the count at 31 so the next addition has to be deliberate.

## Left for FFS

Three questions this sub-feature surfaced and deliberately did not answer:

- **Should parts 3 and 4 be required to submit?** Decided on 2026-09-11 that they are not, on the
  grounds that the legacy form demands neither and requiring them would block a protocol the
  paper form would have accepted.
- **Should the boundary landmarks be required?** `probestrecke.untere` and `probestrecke.obere`
  describe each end of the stretch in words. They carry no asterisk, so they are not required, but
  a landmark is what somebody uses to find the same stretch next year and a coordinate alone is
  harder to stand in front of.
- **Should `ausruestung.leistung` be required?** The device and its construction are marked; its
  power output is not.

## The FormVersion table

`app/formular/felder.py` assigns it to feature 11, reasoning that feature 11 is the first thing
needing the stored rules rather than only the field names. That premise does not survive contact
with the build: the rules turned out to be Python functions, not rows of JSON, so the table would
have nothing to hold. Its real job is ADR 0004, keeping an old protocol renderable after the form
changes, which earns its place when a second form version exists. Left out of scope with the
reasoning recorded rather than dropped quietly.
