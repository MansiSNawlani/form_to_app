...
**Status:** built, reviewed on both axes, walked through against the running application, and
logged. Merged by pull request.
...
Both axes were run against the branch, and four findings were repaired before the work
commit.

1. **One retry policy, not two.** `sollWiederholen` had been copied byte for byte out of
   `pruefliste/abfragen.ts`. It moved to `api/wiederholen.ts`, and the typecheck then found
   a third caller nobody had mentioned, `pruefung/abfragen.ts`, which had been importing the
   queue's copy under an alias. `entwurf/abfragen.ts` keeps a function of the same name and
   was left alone: it gives up on `PROTOKOLL_NICHT_GEFUNDEN` rather than on a refusal, so it
   is a different rule that happens to share a name.

2. **A live region that fell silent.** An earlier fix for a duplicated count left the count
   blank while nothing was being searched for, which meant clearing the search box announced
   nothing at all: somebody using a screen reader was told the list had narrowed and never
   that it had come back. The count is now the only one on the screen, always present, and
   the browser test asserts it after the reset button is pressed.

3. **Over-commenting.** `suche.ts` was 25 comment lines around a one-line filter, with the
   same prose restated in its test. `coding-standards.md` names this as a common AI tell.

4. **A shared class styled from a feature stylesheet**, and an address dropped unescaped into
   a `RegExp` in the browser tests.

## What looking at the screen found that no test did

Every check written for this feature asks whether a control exists and whether it is named,
and all of them passed on a screen whose search row had no padding at all: the label sat
hard against the top and left edges of the card while the table below it was properly inset.
`.card` on this project is a background, a border and a radius, and every screen inside one
supplies its own inset, which this one had not. The loading line and the error box had the
same fault.

This is the second time the fault `coding-standards.md` records from feature 12b has
recurred, and it recurred with screenshots taken and looked at. Spacing is the category that
survives both a green suite and a glance.

## One thing this feature found and did not fix

`backend/app/benutzer/regeln.py` and the extracted option list disagree about which
Regierungspraesidium is which. The hand-written table says 1 is Stuttgart; the list generated
from the legacy form says 1 is Karlsruhe, and 2 the other way round. Freiburg and Tuebingen
agree.

It is visible rather than theoretical: `befischung benutzer anlegen --regierungspraesidium 1`
prints "1 Stuttgart" and this screen prints "Regierungspraesidium Karlsruhe" for the account
it just made.

This screen follows the extracted list, which is what every other screen in the application
already does and what `pruefung/Uebersichtsleiste.tsx` set the precedent for. The number is
what FiaKa receives, so the form is the authority on what it means and the hand-written table
looks like the one that is wrong. Left alone deliberately: it is a backend change outside
this sub-feature, and feature 13 scopes a regional account's view by this number, so which
accounts already carry which number is a question to answer before editing anything.
