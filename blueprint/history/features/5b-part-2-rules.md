...
Automated, on the final tree on 2026-09-03: `npx tsc --noEmit` clean, `npx oxlint` clean,
`npm test` 152 passed across 7 files, `npm run build` clean. Backend untouched and still
green: `pytest` 24 passed, `ruff` clean, `mypy` clean.

Reviewed on both axes against `main` before merging, once for the feature and once after
the fixes. The spec axis found that step 3's done-when contradicted the guard test over
band `0`; that is fixed and the spec now says how the two reconcile.

Manual browser pass: **Mansi ran steps 1 to 9 on 2026-09-02 and reported them working.**
...
- **Subscribe, do not watch.** The hook sits in the component that renders the whole page,
  so a `useWatch` there re-renders the header, the step bar and every field in the open
  section on each Gewässertyp change. A subscription sees the same changes and re-renders
  nothing. `useNachpruefung` already does this and `ProtokollTitel` keeps its watch in the
  smallest component that needs one. Parts 3 to 6 will add more hooks of this kind, so
  they copy this.

  This was changed mid-build while chasing a dropdown defect, and it did **not** fix it.
  See `.scratch/dropdown-flicker/issues/01-select-picks-option-under-pointer.md`. The
  change is kept on its own merits; do not read it as a fix for that.
