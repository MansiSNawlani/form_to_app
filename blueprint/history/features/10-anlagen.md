...
The two logic-bearing steps ship unit tests, run with the rest: 26 new cases across
`anlagen/store.test.ts` and `anlagen/regeln.test.ts`, in a suite of 773. `tsc`, `oxlint` and
`vite build` were green at every step and again at completion. There is no `Verify` command
yet, so the build and the two test commands were the gate.

Every refusal message was proved by rendering it through i18next with real values rather than
by reading the locale file. That is how the two messages that did not name their file were
caught, and how the missing list formatter that printed "JPG,PNG,WEBP" was found. The German
decimal comma was confirmed the same way.

The browser done-whens of steps 4 to 6 were verified by Mansi rather than by an automated
check, confirmed on 2026-09-07. No browser automation exists on this project and
`coding-standards.md` rules out adding Playwright mid-feature, so the agent side proved the
build, the rules and the strings, and the manual pass covered picking, previews, survival
across a reload and the refusal path.
