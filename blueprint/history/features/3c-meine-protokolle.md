...
Three things landed on this branch that the spec above did not ask for. All were
requested in chat on 2026-09-10, and are recorded here so the archive matches what
actually shipped.

**The browser tab icon.** `frontend/public/favicon.ico`, supplied by LAZBW, plus one
`<link>` in `index.html`. It sits in `public/` rather than being imported, so it keeps
a fixed address: a browser asks for `/favicon.ico` before it has read any markup. One
16 by 16 image, so it will look soft on a high-resolution screen until a larger source
is available.

**The empty state's alignment and width.** Two fixes after seeing it on screen. The
paragraph sat against the left edge because it relied on `margin-inline: auto` while
MUI's Typography carries `margin: 0` and emotion injects later; it is a flex column
now, which no margin reset can undo. Then its `max-width: 46ch` cap was removed,
because capping a sentence at a reading measure wrapped it onto three lines inside a
card several times that wide.

**A protocol is created by the first thing typed into it.** Previously "Neues
Protokoll" wrote a record before anybody had typed a character, so opening one and
changing your mind left an empty protocol in the list; two such drafts had
accumulated in the test account. Now nothing is created until the first change, the
save indicator says so while that is true, and leaving beforehand asks mildly. `neu`
travels through the same route parameter as a real id so that both addresses match
one route, which is what lets the address swap without React Router rebuilding the
form and taking the cursor with it. `ProtokollAnlegenFehler` and its strings went
with the loader that needed them.

This last one is not a UI change and would normally have been its own feature. It was
folded in here by explicit request, with the risk stated first: it rewires the
automatic save. The existing save tests stayed green throughout, and
`verlaesstProtokoll`, `istNeu` and `leererEntwurf` shipped with their own.

## Evidence

- `npm test` from `frontend/`: 871 pass, 30 files.
- `pytest` from `backend/`: 293 pass, unchanged; this feature touched no backend code.
- `npm run build` and `npm run lint`: clean.
- The list endpoint exercised against the running stack on 2026-09-09: the summary
  shape, the ordering, an untouched draft answering null for all five display values,
  `DELETE` answering 204, the address then answering 404, and an unauthenticated list
  answering 401.
- Confirmed in the browser by the user on 2026-09-10: the populated list with its
  count line, both delete confirmations, the empty state after the last delete, the
  new protocol not being created until the first keystroke, the mild dialog on leaving
  an untouched one, the address swapping without losing the cursor, a reload finding
  the protocol, and an existing protocol still saving as before.
