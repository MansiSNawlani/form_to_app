...
Eleven things were built differently from the spec above. All were kept after review; this
section is the record, because a spec that no longer describes what was built is worse than
no spec. The last four came out of the two-axis review at the end of the build.

| Change | Was | Is | Why |
|---|---|---|---|
| **`weiter.ts` ships in step 3** | Listed under step 4, the guard | Built with the login page | The login page is what performs the redirect, so the check that stops a crafted link bouncing somebody off this site ships with the code that could do it, not one step later. |
| **The login page stands outside the app shell** | Inside `Layout`, with header and footer | Its own centred card, no shell | A header carrying an account area, on the one page where nobody has an account, reads as a mistake, and there is no navigation for the skip link to skip. The theme toggle and the Impressum and Datenschutz links stay on the page: the first because somebody needing the light theme needs it before signing in, the second because a public authority must make them reachable everywhere. |
| **The LAZBW logo replaces the lettered mark** | Not in the spec at all | `frontend/src/assets/lazbw.png`, in the header and on the card | The green "FFS" tile was a placeholder from feature 1a and read as unfinished. LAZBW supplied the file; the white background was stripped to transparent and the margin trimmed, because a solid white box behind a logo shows on the dark theme. The original file is preserved in commit `1248347`. |
| **The footer's form version was removed** | Not in scope | Gone, with its hard-coded constant | It printed on every screen including the login page, claiming to describe the whole application. A version describes one protocol, and this portal is to carry several forms in time. It is still shown on the protocol itself, read from the open draft. |
| **The placeholder identity went at step 3, not step 5** | Step 5's job | Removed as soon as a login page existed | "M. Bergmann, Einreicher" was harmless while nothing could be signed in and a lie the moment it could, because it told somebody who was not signed in that they were. |
| **`/` redirects to plain `/anmeldung`** | `?weiter=%2F` | No parameter for the home page | It is where signing in lands anyway, and the clean address is better for the most ordinary arrival there is. |
| **The `/protokolle/neu` loader checks the session itself** | Not foreseen | `ensureQueryData(sitzungsAbfrage)` before creating a draft | Loaders run before anything renders, and React Router runs matched loaders together rather than parent first, so the guard cannot stop that one. Without it, a signed-out stranger opening the address had an empty draft created and left in their browser. One shared cache entry, so it is not a second copy of the session. |
| **`.visually-hidden` moved to `shell.css`** | Lived in `protokoll.css` | Lives in `shell.css` | The header needs it on every screen. It only worked everywhere by luck: one merged stylesheet in a build, and whichever modules happened to have loaded in development. |
| **Being signed out carries how it happened** | One `abgemeldet` state | `grund: 'antwort' | 'fehler'` | A backend that could not be reached was being reported to the person as an expired session. Two different problems were wearing one message. The mapping is now the plain function `sitzungAus`, with tests, because both wrong answers there mislead. |
| **502, 503 and 504 read as a service that is down** | Not considered | Mapped to `NETZWERK_FEHLER` | `fetch` rejects only when nothing answers at all, so a stopped backend behind a working proxy was reported as an answer we could not understand. Verified against the real dev proxy, which answers 502. |
| **Two duplications removed after review** | Two copies each | `useFehlertext` and `Anmeldefeld` | The error-rendering ternary had been written in two components and had already drifted apart in whitespace. The login form's two fields re-implemented the aria wiring `protokoll/felder/rahmen.ts` already owns, so those helpers are reused rather than restated. |

## Evidence, 2026-09-08

Recorded here because the done-when clauses ask for it and nothing else in the branch shows it.

- `POST /api/v1/anmeldung` with a real account returns 200, the account body matching
  `api/typen.ts` field for field, and `set-cookie: befischung_sitzung=...; HttpOnly;
  Max-Age=28800; Path=/; SameSite=lax; Secure`.
- A wrong password returns 401 `ANMELDUNG_FEHLGESCHLAGEN`, whose German sentence is what the
  page prints.
- `GET /api/v1/ich` with no cookie returns 401 `NICHT_ANGEMELDET`, which is what the hook turns
  into the signed-out state.
- With the backend stopped, the dev proxy answers 502, which the client reports as a service
  that could not be reached rather than as a bad password.
- `npm test` 807 passing across 24 files; `npm run build` and `npx tsc -b` clean; `oxlint` clean.
- The browser paths, including the guard, the return to a deep-linked section, sign-out, and
  the draft surviving it, were walked manually by the user and confirmed working.

### Known gaps at completion

- The six German role labels are this build's wording, not FFS's. Worth confirming with them.
- `--logo-yellow` in `theme.css` is now used by nothing: it only ever coloured the placeholder
  mark's underline. Left in place as a documented brand constant.
- The home page still promises a list of protocols it does not have, so a draft can only be
  reached by its address. That list is feature 3.
- Browser evidence is the user's, not an automated check. Playwright is deliberately not
  installed; `coding-standards.md` forbids adding it mid-feature.
