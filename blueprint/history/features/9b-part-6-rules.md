...
## Found in the manual walk-through, 2026-09-06

### Removing a row left the messages of the row that moved

Reported by Mansi while walking the table. Naming Hecht in row 2 raised the duplicate message
correctly, but deleting row 1 shifted row 2 up and left a message behind, on a row that was now
blank, with the text changed to the 0+ rule. Separately, deleting row 1 of three left the "Art 3"
lines in the list after row 3 had emptied and hidden. Both cleared on a page reload.

One cause. A removal writes the shifted values with `setValue`, and neither mechanism that
normally keeps a message honest reaches that. React Hook Form validates a field when the user
leaves it, and nobody left these; `nachzupruefen` deliberately never rechecks the field that
changed, because while somebody is typing that field is already React Hook Form's own job. So the
cells were blanked and their complaints stayed on screen.

The changing text has the same explanation. The shift writes one cell at a time, so mid-shift a
row holds old counts beside new blanks, and `useNachpruefung` fired a check against that half
state. The message it wrote was true at that instant and nothing came back to correct it.

Fixed by rechecking every moved cell once, after the last write rather than during it:

    void trigger(schreibvorgaenge.map(({ pfad }) => pfad))

Once and afterwards, both deliberately. The row rules read a whole row and the duplicate rule
reads every row, so a check run partway through the shift judges a table that is half old and
half new, which is how the wrong message arrived in the first place. `trigger` both sets and
clears, so a blanked cell loses its message and a genuinely wrong value that moved up keeps one.

**Not covered by an automated test**, and this is a gap worth naming rather than hiding. The
behaviour is `setValue` plus `trigger` against a live form, and `AGENTS.md` records that vitest
runs here in a node environment with no DOM. Proving it would mean adding jsdom and a testing
library, which `coding-standards.md` makes a deliberate decision rather than something to add
mid-feature. Verified by hand instead, by Mansi, on both reported cases.
