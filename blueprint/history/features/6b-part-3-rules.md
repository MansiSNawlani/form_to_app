...
## Manual verification, 2026-09-03

Walked by Mansi against the dev server, since the on-screen done-whens are the ones
no automated gate here can reach. Playwright is not installed and
`coding-standards.md` rules out adding it mid-feature.

Confirmed working:

- Section 3 renders as 6a left it, with the new total line under each of the six groups
- The total updates as the user types and stays inside its own group
- The message clears the moment a group reaches 100
- An unparseable share reports on that share, and the total stays quiet, which is the
  bug the code review caught
- A reloaded draft sitting at 83 shows its message on load, without touching anything
- Light and dark both readable, for the muted total, the red message and the green
  complete state
- Keyboard focus visible, and the total is skipped in the tab order

One question raised and answered rather than changed: once a share in a group has been
left, the message updates live on every keystroke rather than waiting for another blur.
That is React Hook Form's `onTouched` mode, which the form has used since 4a, and every
field in parts 1 and 2 behaves the same way. It is also what lets the message vanish the
instant a correction reaches 100. Left as is deliberately.
