...
**Status:** built, branch-reviewed on both axes, proved against the running application
with 28 Playwright tests, and merged.
...
7. **The crumb is bold and in the accent colour, not the sub-line's grey.** The first person
   ...
   **Left alone deliberately, and confirmed with the user on 2026-09-22:** the header's
   Pruefliste tab still goes to the unfiltered queue, and still clears the filters even when
   the reader is already on the Pruefliste. It is site-wide navigation, "take me to the
   queue" is a reasonable thing for it to mean, and making one word mean two destinations
   depending on where the reader has been is its own surprise. The crumb is the control that
   remembers.
