...
Added on completion, 2026-09-01.

### How it was verified

| Claim | Evidence |
|---|---|
| The draft store behaves under failure | `npm test`, 9 tests: round trip, missing draft, corrupt JSON, a write that throws, listing order, foreign keys ignored |
| It compiles and builds | `npx tsc -b` and `npm run build`, both clean |
| Standards and lint | `npm run lint` clean, plus a two-axis review (standards and spec) whose findings were fixed before the merge |
| The screen actually works | Manual pass by the user against the running dev server: draft creation, the save indicator, section switching, focus landing on the card, the Anlass answer surviving a reload, keyboard navigation, dark mode, and the not-found page |

No browser automation was used. Playwright is not installed and `coding-standards.md` says not to add it mid-feature, so the behavioural criteria rest on the manual pass rather than on a script.

### Deviations from the spec as written

1. **The test runner was added inline rather than by a separate `/tests` run.** Vitest, node environment, with `npm test` recorded in `AGENTS.md`. The store takes its storage as an argument, so its tests need no DOM.
2. **Storage keys are scanned instead of an index key being maintained.** The spec's data contract called for "one key per draft plus an index key". An index is a second write that can fail on its own, and a draft missing from it would be invisible although its data is intact. `listEntwuerfe` scans the prefix instead.
3. **Form controls are MUI, not native.** The field was first built as a native `select` styled from the tokens, matching the mockup. On 2026-09-01 the user decided that MUI controls are used wherever MUI has one, since 338 fields will copy whatever the first one does. Recorded in `coding-standards.md`. It cost about 88KB of bundle.
4. **Focus lands on the section card, not a section heading.** The card is labelled with the section title, so it announces correctly. The mockup has no per-section heading to focus, only the step bar and the fieldset legends.
5. **Small additions the router needed:** a catch-all not-found route, the placeholder button on `/` becoming a real link, and an invalid section number redirecting to section 1.

### Carried into 4b

- The whole 97KB option-list file is bundled for one 6-entry dropdown, which pushes the build past Vite's 500KB warning. Split it per list when the 722-entry `probestrecke.monitoringnummer` arrives.
- `zod` is installed but unused until 4c, and `listEntwuerfe` has no caller until feature 3.
- The four source discrepancies recorded under "Notes for the AI" above still need acting on, in particular the missing `bearbeiter.ort` field, which is worth raising with FFS.
- Component and file names still mix German and English (`ProtokollSeite`, `SpeicherAnzeige`, `typen.ts`). Local variables and props were made English during review; the file-name convention was left alone and is worth settling before 4b copies it.
