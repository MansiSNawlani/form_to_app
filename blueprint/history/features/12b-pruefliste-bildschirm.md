...
**Status:** built, branch-reviewed on both axes, proved against the running application with
15 Playwright tests, and merged.
...
Five things the code settles which the spec left open. Recorded here rather than removed,
because 12c and 12d inherit all five.

1. **`istGefiltert` is a named, tested function, not a check on the query string.** The first
   version asked "is anything in the address bar?", which counts being on page 2 as being
   filtered. An empty page past the end then told a reader who had narrowed nothing that
   nothing matched their filters, and offered to reset filters they never set. The page
   number is in the same string as the filters and is not one of them.

2. **The empty state hangs off `gesamt`, not off this page's rows.** A page past the end also
   comes back with no rows and is not empty in the sense that state means.

3. **Only the search box replaces its history entry.** Everything else pushes, so Back undoes
   the last thing done to the list rather than leaving the screen. A blanket replace, which is
   what the first version did, made Back skip the whole screen.

4. **`darfPruefen` in `auth/startseite.ts` is the browser's single answer to "who is FFS
   staff"**, the same way `FFS_ROLLEN` is the backend's. The header reads it to decide whether
   to draw the Pruefliste link and `startseite` reads it to decide where to land. It is never
   a permission: the endpoint refuses regardless, and hiding a link is not security.

5. **`NICHT_ANGEMELDET` is not retried either**, alongside the `ROLLE_FEHLT` the spec named.
   Both are settled answers about the caller rather than about the request.

## What browser automation cost and bought

Playwright was added here, at the user's request, and `coding-standards.md` was updated from
"not installed" to the arrangement now in place. Two things are worth carrying forward.

**The tests address controls by accessible role and name.** A selector that cannot find "the
combobox named Status" is a control a screen reader cannot announce either, so the suite
carries part of the accessibility requirement rather than restating it. That is what holds
the fix to the four unnamed dropdowns the branch review found.

**A screenshot still earns its place.** All twelve tests passed while the Jahr and Anlass
dropdowns were drawing as empty boxes and the search placeholder was cut off mid-word.
Tests check what somebody thought to ask; looking at the page showed what was actually there.
