...
**Status:** built, all ten steps done, reviewed on two axes
...
**The visual check was never made.** Steps 8 and 9 both end with "screenshots in both
themes", and no browser was opened: Playwright is not installed and
`coding-standards.md` says not to add it mid-feature. What stands in its place is the
whole workflow driven against the running stack over HTTP on 2026-09-14, which proves the
behaviour and says nothing about the look.

So nobody has yet seen the change-request notice above the form, or the Ueberarbeiten link
on Meine Protokolle, in either theme. **Mansi accepted this knowingly on 2026-09-15** and
took the visual check on themselves, to be raised as a `/fix` if anything looks wrong.

It is written here rather than ticked quietly because the next person to read this archive
would otherwise believe those two done-whens were met.
