...
function Verlauf({ protokoll }: { protokoll: Entwurf }) {
  ...
  return (
    <Panel titel={t('protokoll.verlauf.titel')}>
      {/* Said out loud rather than only drawn, the way the protocol's own page
          announces its loading state. */}
      {isPending && (
      ...
      {/* Never softened into "nothing has happened yet". A history that failed to
          load and a protocol nothing was ever done to look identical on screen if
          this says the wrong one, and the second is a lie about a protocol that
          has certainly at least been submitted. */}
      ...
      {eintraege !== undefined && (
        <ul className="history">
          ...
          {/* The oldest line, and the only one that is not a recorded event.
              ...
              server's history can never begin before Eingereicht. Both halves of
              this are certainly true and already on the payload: the draft was
              made by the account that owns it, on that day. */}
          <Zeile
