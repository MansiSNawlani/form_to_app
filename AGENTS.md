...
**Database migrations** (also from `backend/`, added in feature 2a, needs the
database running). The scripts themselves live in `database/migrations/`, which is
where `coding-standards.md` puts them; `alembic.ini` sits here because this is
where the virtual environment is.

- Which migration the database is on: `alembic current`
...
An autogenerated migration is a draft. Alembic does not see check constraints,
gets server defaults wrong, and generates a drop for any model it was not told
about, so every generated file is read before it is kept.

Migrations are never run automatically when the application starts. Where the
service runs as more than one container they would all migrate at once. Applying
